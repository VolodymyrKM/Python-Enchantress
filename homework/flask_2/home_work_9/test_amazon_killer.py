...
from main_amazon_killer import main_amazon_killer as app
...
def test_create_product_carts(store_app):
    ...
    response = store_app.get(f'/cart/{cart_id}')
...
def test_update_invalid_cart_input(store_app, value):
    response = store_app.put(f'/cart/{value}')
...
def test_del_cart_user_invalid(store_app, value):
    response = store_app.delete(f'/cart/{value}')
